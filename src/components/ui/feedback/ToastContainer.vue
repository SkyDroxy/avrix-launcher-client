<template>
  <div class="fixed z-50 inset-0 pointer-events-none flex flex-col items-end gap-2 p-4 sm:p-6">
    <transition-group
      name="toast-fade"
      tag="div"
        ...
        <div class="flex-1 min-w-0 flex flex-col">
          ...
          >
            <div v-for="(v, k) in t.meta" :key="k" class="flex justify-between gap-2">
              <span class="truncate">{{ k }}</span><span class="truncate font-mono">{{ v }}</span>
            </div>
          </div>
        </div>
        <UiButton
